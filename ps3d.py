...
target2 = 'atgaatgcactggatgtaaatgcag'
    ...
    for x in range(0, len(firstMatch)):
        for y in range(0, len(secondMatch) ):
            if firstMatch[x] + length + 1 == secondMatch[y]:
                nearMiss = nearMiss + (firstMatch[x], )
    return nearMiss
...
def subStringMatchExactlyOneSub(target, key):
    ...
    for miss in range(0,len(key)):
        ...
        # match1 and match2 are tuples of locations of start of matches
        # for each substring in target
        #MatchAll is for the ones that have the entire key matched. 
        match1 = subStringMatchExact(target,key1)
        match2 = subStringMatchExact(target,key2)
        matchAll = subStringMatchExact(target, key)
        # when we get here, we have two tuples of start points
        # need to filter pairs to decide which are correct
        filtered = constrainedMatchPair(match1,match2,len(key1))
        for nearMiss in range(0, len(filtered)):
            hits = 0
            matchFound = False
            while hits < len(matchAll) and matchFound == False:
                if filtered[nearMiss] == matchAll[hits]:
                    matchFound = True
                hits += 1
            if matchFound == False:
                allAnswers = allAnswers + (filtered[nearMiss], )
            matchFound = False
        print 'match1',match1
        print 'match2',match2
        print 'possible matches for',key1,key2,'start at',filtered
